...
class SingleAssetOrderExecutionSimple(Simulator[Order, SAOEState, float]):
    ...
    def _metrics_collect(
        ...
        market_vol ,
        market_price ,
        amount ,  # intended to trade such amount
        exec_vol ,
    )  :
    ...
    def _get_ticks_slice(self, start , end , include_end  = False)  :
        if not include_end:
            end = end - EPS_T
        return self.ticks_index[self.ticks_index.slice_indexer(start, end)]

    @staticmethod
    def _dataframe_append(df , other )  :
...
def price_advantage(
    ...
    res_wo_nan  = np.nan_to_num(res, nan=0.0)
