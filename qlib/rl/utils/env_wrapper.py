...
class BaseEnvWrapper(
    ...
    def __init__(self)  :
        self.status  = cast(EnvWrapperStatus, None)
...
class EnvWrapper(
    ...
    def __init__(
        ...
        self.logger  = logger or LogCollector()
        self.status  = cast(EnvWrapperStatus, None)
    ...
    def step(self, policy_action , **kwargs )     :
        """Environment step.
...
class CollectDataEnvWrapper(BaseEnvWrapper[InitialStateType, StateType, ActType, ObsType, PolicyActType]):
    ...
    def step(self, policy_action  = None, **kwargs )  :
        self.status["cur_step"] += 1
