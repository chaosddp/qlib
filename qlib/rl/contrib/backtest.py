...
def _set_env_for_all_strategy(executor )  :
...
def _convert_indicator_to_dataframe(indicator )  :
    record_list = []
    ...
    records  = pd.concat(record_list, 0).reset_index().rename(columns={"index": "instrument"})
    records = records.set_index(["instrument", "datetime"])
    return records


def _generate_report(decisions , report_dict )  :
    report = {}
